    ...
    $.ajax({
        ...
        success: function (json) {
            $.each(json, function (i, item) {
                $('#articleCategories').append('<option categoryId="' + item.categoryId + '">' + item.name + '</option>');
                $('#addCategories').append('<option categoryId="' + item.categoryId + '">' + item.name + '</option>');
            });
        }
    });
...
document.getElementById("articleCategories").onchange = function () {
    ...
    if (categoryId === "") {
        ...
        var url = "/api/article/list/sort/" + categoryId;
    }
    // 填充博文分类信息
    $.ajax({
        url: url,
    ...
    $.ajax({
        ...
        success: function (json) {
            ...
            if (json.top == true) {
                ...
                document.getElementById("articleTop").checked = false;
            }
            // 填充分类数据
            $.ajax({
                url: "/api/category/list",
                ...
                success: function (json) {
                    ...
                    $.each(json, function (i, item) {
                        $('#updateCategories').append('<option categoryId="' + item.categoryId + '">' + item.name + '</option>');
                    });
                }
            });
            ...
            for (var i = 0; i < select.options.length; i++) {
                if (select.options[i].innerHTML == json.categoryName) {
                    					...
                    					// alert(json.categoryId);
                    select.options[i].selected = true;
                    break;
                }
            }
            $('#articlePicture').val(json.pictureUrl);
            $('#articleContent').val(json.article);
        }
    });
